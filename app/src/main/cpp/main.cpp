...
void android_main(struct android_app* state) {
  ...
  Engine* test_engine = Engine::create();
  SwapChain* swapChain = test_engine->creatSwapChain(engine.app->window);
  Renderer* renderer = test_engine->createRenderer();
  Scene* scene = test_engine->createScene();
  View* view = test_engine->createView();
  view->setScene(scene);
  EntityManager& entityManager = test_engine->getEntityManager();
  Entity cameraEntity = entityManager->create();
  Camera* camera = test_engine->createCamera(cameraEntity);
  ...
  VertexBuffer* vb = VertexBuffer::Builder()
          ...
          .build(*test_engine);
  vb->setBufferAt(*test_engine, 0, VertexBuffer::BufferDescriptor(TRIANGLE_VERTICES, 36, nullptr));
  IndexBuffer* ib = IndexBuffer::Builder()
          .indexCount(3)
          .bufferType(IndexBuffer::IndexType::USHORT)
          .build(*test_engine);
  ib->setBuffer(*test_engine, IndexBuffer::BufferDescriptor(TRIANGLE_INDICES, 6, nullptr));
